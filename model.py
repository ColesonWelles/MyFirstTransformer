...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Applying the sin to the even positions
        pe[:, 0::2] = torch.sin(position * div_term)
        # Appling the cos to the odd positions
        pe[:, 1::2] = torch.cos(position * div_term)
        
        pe = pe.unsqueeze(0) # Adding a new dimension to this pe -> (1, seq_len, d_model)

        """ 
        Reserving this tensor in this module's buffer. When you have a tensor that you 
        want to keep within the module, not as a learned parameter, but you still want
        it to be saved when you save the model file then register it as a buffer to 
        save it with the state of the model.  
        """
...
class FeedForwardBlock(nn.Module):
    ...
    def __ini__(self, d_model: int, d_ff: int, dropout: float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock,
                 feed_forward_block: FeedForwardBlock, droput: float) -> None:
        super().__init__
        self.self_attention_block = self_attention_block
        self.cross_attention_block = cross_attention_block
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ResidualConnection(droput) for _ in range(3)])
